...
exports.verificationPublicJwkForIndyDid = verificationPublicJwkForIndyDid;
...
function isSelfCertifiedIndyDid(did, verkey) {
    ...
    const { did: didFromVerkey } = indyDidFromNamespaceAndInitialKey(namespace, core_1.Kms.PublicJwk.fromPublicKey({
        crv: 'Ed25519',
        kty: 'OKP',
        publicKey: core_1.TypedArrayEncoder.fromBase58(verkey),
    }));
...
function indyDidFromNamespaceAndInitialKey(namespace, initialKey) {
    const buffer = core_1.Hasher.hash(initialKey.publicKey.publicKey, 'sha-256');
    const id = core_1.TypedArrayEncoder.toBase58(buffer.slice(0, 16));
    const verkey = core_1.TypedArrayEncoder.toBase58(initialKey.publicKey.publicKey);
...
async function verificationPublicJwkForIndyDid(agentContext, did) {
    ...
    if (!publicJwk.is(core_1.Kms.Ed25519PublicJwk)) {
        throw new core_1.CredoError('Expected #verkey verification mehod to be of type Ed25519');
    }
