...
const didCommMessageRoleMapping = {
    [V01_02MigrationCredentialRole.Issuer]: {
        proposalMessage: repository_1.DidCommMessageRole.Receiver,
        offerMessage: repository_1.DidCommMessageRole.Sender,
        requestMessage: repository_1.DidCommMessageRole.Receiver,
        credentialMessage: repository_1.DidCommMessageRole.Sender,
    },
    [V01_02MigrationCredentialRole.Holder]: {
        proposalMessage: repository_1.DidCommMessageRole.Sender,
        offerMessage: repository_1.DidCommMessageRole.Receiver,
        requestMessage: repository_1.DidCommMessageRole.Sender,
        credentialMessage: repository_1.DidCommMessageRole.Receiver,
    },
};
...
function getCredentialRole(credentialRecord) {
    ...
    if (credentialRecord.state === CredentialState_1.CredentialState.Done) {
...
async function updateIndyMetadata(agent, credentialRecord) {
    ...
    const metadata = new core_1.Metadata(rest);
...
async function moveDidCommMessages(agent, credentialRecord) {
    ...
    const didCommMessageRepository = agent.dependencyManager.resolve(repository_1.DidCommMessageRepository);
    for (const messageKey of credentialRecordMessageKeys) {
        ...
        if (message) {
            ...
            const didCommMessageRecord = new repository_1.DidCommMessageRecord({
