...
const repository_1 = require("../../modules/oob/repository");
/**
...
async function extractDidDocument(agent, connectionRecord) {
    ...
    if (oldOurDidDocJson) {
        const oldOurDidDoc = core_1.JsonTransformer.fromJSON(oldOurDidDocJson, connections_1.DidDoc);
        ...
        const { didDocument: newOurDidDocument } = (0, helpers_1.convertToNewDidDocument)(oldOurDidDoc);
        ...
        if (!ourDidRecord) {
            ...
            ourDidRecord = new core_1.DidRecord({
            ...
            ourDidRecord.metadata.set(core_1.DidRecordMetadataKeys.LegacyDid, {
                unqualifiedDid: oldOurDidDoc.id,
                didDocumentString: core_1.JsonEncoder.toString(oldOurDidDocJson),
            });
    ...
    if (oldTheirDidDocJson) {
        const oldTheirDidDoc = core_1.JsonTransformer.fromJSON(oldTheirDidDocJson, connections_1.DidDoc);
        ...
        const { didDocument: newTheirDidDocument } = (0, helpers_1.convertToNewDidDocument)(oldTheirDidDoc);
        // Maybe we already have a record for this did because the migration failed previously
        // NOTE: in 0.3.0 the id property was updated to be a uuid, and a new did property was added. As this is the update from 0.1 to 0.2,
        // the `id` property of the record is still the did here.
        ...
        if (!theirDidRecord) {
            ...
            theirDidRecord = new core_1.DidRecord({
                // NOTE: in 0.3.0 the id property was updated to be a uuid, and a new did property was added. Here we make the id and did property both the did.
                // In the 0.3.0 update the `id` property will be updated to an uuid.
            ...
            theirDidRecord.metadata.set(core_1.DidRecordMetadataKeys.LegacyDid, {
                unqualifiedDid: oldTheirDidDoc.id,
                didDocumentString: core_1.JsonEncoder.toString(oldTheirDidDocJson),
            });
...
async function migrateToOobRecord(agent, connectionRecord) {
    ...
    const oobRepository = agent.dependencyManager.resolve(repository_1.OutOfBandRepository);
    const connectionRepository = agent.dependencyManager.resolve(connections_1.ConnectionRepository);
    const untypedConnectionRecord = connectionRecord;
    ...
    if (oldInvitationJson) {
        const oldInvitation = core_1.JsonTransformer.fromJSON(oldInvitationJson, connections_1.ConnectionInvitationMessage);
        agent.config.logger.debug('Found a legacy invitation in connection record. Migrating it to an out of band record.');
        const outOfBandInvitation = (0, converters_1.convertToNewInvitation)(oldInvitation);
        ...
        const recipientKeyFingerprints = outOfBandInvitation
            ...
            .map((didKey) => core_1.DidKey.fromDid(didKey).publicJwk.fingerprint);
        ...
        if (!oobRecord) {
            ...
            oobRecord = new repository_1.OutOfBandRecord({
                role: oobRole,
                state: oobState,
        ...
        const [invitationDid] = [
            ...
            ...oobRecord.outOfBandInvitation.getInlineServices().map(helpers_2.outOfBandServiceToInlineKeysNumAlgo2Did),
        ];
