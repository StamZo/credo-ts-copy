...
async function getProofRole(agent, proofRecord) {
    ...
    if (!didCommMessageRecord) {
        throw new core_1.CredoError(`Unable to determine the role of the proof exchange record with id ${proofRecord.id} without any didcomm messages and state abandoned/done`);
    }
    ...
    const roleStateMapping = {
        [proofs_1.V2ProposePresentationMessage.type.messageName]: {
            [repository_1.DidCommMessageRole.Sender]: proofs_1.ProofRole.Prover,
            [repository_1.DidCommMessageRole.Receiver]: proofs_1.ProofRole.Verifier,
        },
        [proofs_1.V2RequestPresentationMessage.type.messageName]: {
            [repository_1.DidCommMessageRole.Sender]: proofs_1.ProofRole.Verifier,
            [repository_1.DidCommMessageRole.Receiver]: proofs_1.ProofRole.Prover,
        },
    };
