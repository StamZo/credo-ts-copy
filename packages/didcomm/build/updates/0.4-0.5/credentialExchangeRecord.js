...
async function getCredentialRole(agent, credentialRecord) {
    ...
    if (credentialRecord.state === credentials_1.CredentialState.Done) {
    ...
    if (!didCommMessageRecord) {
        throw new core_1.CredoError(`Unable to determine the role of the credential exchange record with id ${credentialRecord.id} without any didcomm messages and state abandoned`);
    }
    ...
    const roleStateMapping = {
        [credentials_1.V2OfferCredentialMessage.type.messageName]: {
            [repository_1.DidCommMessageRole.Sender]: credentials_1.CredentialRole.Issuer,
            [repository_1.DidCommMessageRole.Receiver]: credentials_1.CredentialRole.Holder,
        },
        [credentials_1.V2ProposeCredentialMessage.type.messageName]: {
            [repository_1.DidCommMessageRole.Sender]: credentials_1.CredentialRole.Holder,
            [repository_1.DidCommMessageRole.Receiver]: credentials_1.CredentialRole.Issuer,
        },
