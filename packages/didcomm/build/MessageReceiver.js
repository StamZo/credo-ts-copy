...
const services_1 = require("./modules/connections/services");
...
let MessageReceiver = class MessageReceiver {
    ...
    async receiveMessage(inboundMessage, { session, connection, contextCorrelationId, receivedAt, } = {}) {
        ...
        try {
            if (this.isEncryptedMessage(inboundMessage)) {
                ...
                throw new core_1.CredoError('Unable to parse incoming message: unrecognized format');
            }
        }
        finally {
    ...
    async transformMessage(message) {
        ...
        try {
            messageTransformed = core_1.JsonTransformer.fromJSON(message, MessageClass);
        }
        catch (error) {
            ...
            throw new errors_1.ProblemReportError(`Error validating message ${message['@type']}`, {
                problemCode: models_1.ProblemReportReason.MessageParseFailure,
            });
        }
    ...
    async sendProblemReportMessage(agentContext, message, connection, plaintextMessage) {
        ...
        const problemReportMessage = new messages_1.ProblemReportMessage({
            description: {
                en: message,
                code: models_1.ProblemReportReason.MessageParseFailure,
            },
        });
...
exports.MessageReceiver = MessageReceiver = __decorate([
    (0, core_1.injectable)(),
    __param(6, (0, core_1.inject)(core_1.InjectionSymbols.AgentContextProvider)),
