...
class ConnectionInvitationMessage extends AgentMessage_1.AgentMessage {
    ...
    constructor(options) {
        ...
        if (options) {
            ...
            if (options.did && (options.recipientKeys || options.routingKeys || options.serviceEndpoint)) {
                throw new core_1.CredoError('either the did or the recipientKeys/serviceEndpoint/routingKeys must be set, but not both');
            }
        }
    }
    /**
    ...
    static fromUrl(invitationUrl) {
        ...
        if (typeof encodedInvitation === 'string') {
            ...
            const invitation = core_1.JsonTransformer.fromJSON(invitationJson, ConnectionInvitationMessage);
            return invitation;
        }
        throw new core_1.CredoError('InvitationUrl is invalid. Needs to be encoded with either c_i, d_m, or oob');
    }
}
