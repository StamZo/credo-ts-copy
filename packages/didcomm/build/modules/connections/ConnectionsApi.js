...
let ConnectionsApi = class ConnectionsApi {
    ...
    async acceptOutOfBandInvitation(outOfBandRecord, config) {
        ...
        if (ourDid && config.routing) {
            throw new core_1.CredoError(`'routing' is disallowed when defining 'ourDid'`);
        }
        ...
        if (protocol === models_2.HandshakeProtocol.DidExchange) {
            ...
            if (ourDid) {
                throw new core_1.CredoError('Using an externally defined did for connections protocol is unsupported');
            }
            ...
            if (!routing) {
                throw new core_1.CredoError('Routing is required for connections protocol');
            }
            ...
            throw new core_1.CredoError(`Unsupported handshake protocol ${protocol}.`);
        }
    ...
    async acceptRequest(connectionId) {
        ...
        if (!connectionRecord) {
            throw new core_1.CredoError(`Connection record ${connectionId} not found.`);
        }
        if (!connectionRecord.outOfBandId) {
            throw new core_1.CredoError(`Connection record ${connectionId} does not have out-of-band record.`);
        }
        const outOfBandRecord = await this.outOfBandService.findById(this.agentContext, connectionRecord.outOfBandId);
        if (!outOfBandRecord) {
            throw new core_1.CredoError(`Out-of-band record ${connectionRecord.outOfBandId} not found.`);
        }
    ...
    async acceptResponse(connectionId) {
        ...
        if (connectionRecord.protocol === models_2.HandshakeProtocol.DidExchange) {
            if (!connectionRecord.outOfBandId) {
                throw new core_1.CredoError(`Connection ${connectionRecord.id} does not have outOfBandId!`);
            }
            const outOfBandRecord = await this.outOfBandService.findById(this.agentContext, connectionRecord.outOfBandId);
            if (!outOfBandRecord) {
                throw new core_1.CredoError(`OutOfBand record for connection ${connectionRecord.id} with outOfBandId ${connectionRecord.outOfBandId} not found!`);
            }
    ...
    async rotate(options) {
        ...
        if (toDid && options.routing) {
            throw new core_1.CredoError(`'routing' is disallowed when defining 'toDid'`);
        }
...
exports.ConnectionsApi = ConnectionsApi = __decorate([
    ...
    __metadata("design:paramtypes", [MessageHandlerRegistry_1.MessageHandlerRegistry,
        ...
        services_1.TrustPingService,
        RoutingService_1.RoutingService,
        core_1.DidRepository,
        core_1.DidResolverService,
