...
let DidRotateService = class DidRotateService {
    ...
    async createRotate(agentContext, options) {
        ...
        const config = agentContext.resolve(ConnectionsModuleConfig_1.ConnectionsModuleConfig);
        ...
        if (didRotateMetadata) {
            throw new core_1.CredoError(`There is already an existing opened did rotation flow for connection id ${connection.id}`);
        }
        let resolvedDid;
        let mediatorId;
        ...
        if (toDid) {
            ...
            mediatorId = (await (0, helpers_1.getMediationRecordForDidDocument)(agentContext, resolvedDid.didDocument))?.id;
            // Otherwise, create a did:peer based on the provided routing
        }
        else {
            if (!routing) {
                throw new core_1.CredoError('Routing configuration must be defined when rotating to a new peer did');
            }
            resolvedDid = await (0, helpers_2.createPeerDidFromServices)(agentContext, (0, helpers_2.routingToServices)(routing), config.peerNumAlgoForDidRotation);
            mediatorId = routing.mediatorId;
        }
        const message = new messages_2.DidRotateMessage({ toDid: resolvedDid.didDocument.id });
        ...
        await agentContext.resolve(ConnectionService_1.ConnectionService).update(agentContext, connection);
        return message;
    }
    async createHangup(agentContext, options) {
        const { connection } = options;
        const message = new messages_2.HangupMessage({});
    ...
    async processRotate(messageContext) {
        ...
        if ((0, core_1.isValidPeerDid)(newDid) && (0, core_1.getNumAlgoFromPeerDid)(newDid) === core_1.PeerDidNumAlgo.GenesisDoc) {
            this.logger.error(`Unable to resolve DID Document for '${newDid}`);
            const response = new messages_2.DidRotateProblemReportMessage({
        ...
        const outboundMessageContext = new models_1.OutboundMessageContext(new messages_2.DidRotateAckMessage({
            threadId: message.threadId,
        ...
        await agentContext.dependencyManager.resolve(core_1.DidRepository).storeReceivedDid(agentContext, {
            did: didDocument.id,
            didDocument,
            tags: {
    ...
    async processRotateAck(inboundMessage) {
        ...
        const didRotateMetadata = connection.metadata.get(ConnectionMetadataTypes_1.ConnectionMetadataKeys.DidRotate);
        if (!didRotateMetadata) {
            throw new core_1.CredoError(`No did rotation data found for connection with id '${connection.id}'`);
        }
        if (didRotateMetadata.threadId !== message.threadId) {
            throw new core_1.CredoError(`Existing did rotation flow thread id '${didRotateMetadata.threadId} does not match incoming message'`);
        }
...
exports.DidRotateService = DidRotateService = __decorate([
    ...
    __metadata("design:paramtypes", [core_1.DidResolverService, Object, core_1.EventEmitter])
], DidRotateService);
