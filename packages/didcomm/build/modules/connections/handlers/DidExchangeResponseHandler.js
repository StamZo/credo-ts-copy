...
class DidExchangeResponseHandler {
    ...
    async handle(messageContext) {
        ...
        if (!recipientKey || !senderKey) {
            throw new core_1.CredoError('Unable to process connection response without sender key or recipient key');
        }
        ...
        if (!connectionRecord) {
            throw new core_1.CredoError(`Connection for thread ID ${message.threadId} not found!`);
        }
        if (!connectionRecord.did) {
            throw new core_1.CredoError(`Connection record ${connectionRecord.id} has no 'did'`);
        }
        const ourDidDocument = await this.didResolverService.resolveDidDocument(agentContext, connectionRecord.did);
        if (!ourDidDocument) {
            throw new core_1.CredoError(`Did document for did ${connectionRecord.did} was not resolved`);
        }
        ...
        if (!ourDidDocument.recipientKeys.find((key) => key.fingerprint === recipientKey.fingerprint)) {
            throw new core_1.CredoError(`Recipient key ${recipientKey.fingerprint} not found in did document recipient keys.`);
        }
        const { protocol } = connectionRecord;
        if (protocol !== models_2.HandshakeProtocol.DidExchange) {
            throw new core_1.CredoError(`Connection record protocol is ${protocol} but handler supports only ${models_2.HandshakeProtocol.DidExchange}.`);
        }
