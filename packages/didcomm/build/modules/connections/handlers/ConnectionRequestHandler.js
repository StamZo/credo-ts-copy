...
class ConnectionRequestHandler {
    ...
    async handle(messageContext) {
        ...
        if (!recipientKey || !senderKey) {
            throw new core_1.CredoError('Unable to process connection request without senderVerkey or recipientKey');
        }
        const parentThreadId = message.thread?.parentThreadId;
        const outOfBandRecord = parentThreadId && (0, core_1.tryParseDid)(parentThreadId)
        ...
        if (!outOfBandRecord) {
            throw new core_1.CredoError(`Out-of-band record for recipient key ${recipientKey.fingerprint} was not found.`);
        }
        if (connection && !outOfBandRecord.reusable) {
            throw new core_1.CredoError(`Connection record for non-reusable out-of-band ${outOfBandRecord.id} already exists.`);
        }
        ...
        if (receivedDidRecord) {
            throw new core_1.CredoError(`A received did record for sender key ${senderKey.fingerprint} already exists.`);
        }
        if (outOfBandRecord.state === OutOfBandState_1.OutOfBandState.Done) {
            throw new core_1.CredoError('Out-of-band record has been already processed and it does not accept any new requests');
        }
