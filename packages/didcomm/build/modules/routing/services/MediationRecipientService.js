...
let MediationRecipientService = class MediationRecipientService {
    ...
    async processMediationGrant(messageContext) {
        ...
        const connectionUsesDidKey = messageContext.message.routingKeys.some(core_2.isDidKey);
        ...
        mediationRecord.routingKeys = messageContext.message.routingKeys.map(core_2.didKeyToVerkey);
    ...
    async processKeylistUpdateResults(messageContext) {
        ...
        // Update connection metadata to use their key format in further protocol messages
        const connectionUsesDidKey = keylist.some((key) => (0, core_2.isDidKey)(key.recipientKey));
        ...
        for (const update of keylist) {
            if (update.action === messages_1.KeylistUpdateAction.add) {
                mediationRecord.addRecipientKey((0, core_2.didKeyToVerkey)(update.recipientKey));
            }
            else if (update.action === messages_1.KeylistUpdateAction.remove) {
                mediationRecord.removeRecipientKey((0, core_2.didKeyToVerkey)(update.recipientKey));
            }
        }
    ...
    async keylistUpdateAndAwait(agentContext, mediationRecord, updates, timeoutMs = 15000 // TODO: this should be a configurable value in agent config
        ...
        const message = this.createKeylistUpdateMessage(updates.map((item) => new KeylistUpdateMessage_1.KeylistUpdate({
            ...
            recipientKey: useDidKey
                ? new core_2.DidKey(item.recipientKey).did
                : core_2.TypedArrayEncoder.toBase58(item.recipientKey.publicKey.publicKey),
        })));
        mediationRecord.assertReady();
        mediationRecord.assertRole(models_2.MediationRole.Recipient);
        ...
        observable
            .pipe((0, core_2.filterContextCorrelationId)(agentContext.contextCorrelationId), 
    ...
    async addMediationRouting(agentContext, routing, { mediatorId, useDefaultMediator = true } = {}) {
        ...
        return {
            ...
            routingKeys: mediationRecord.routingKeys.map((key) => core_1.Kms.PublicJwk.fromPublicKey({ kty: 'OKP', crv: 'Ed25519', publicKey: core_2.TypedArrayEncoder.fromBase58(key) })),
        };
    }
    async removeMediationRouting(agentContext, { recipientKeys, mediatorId }) {
        ...
        if (!mediationRecord) {
            throw new core_2.CredoError('No mediation record to remove routing from has been found');
        }
    ...
    async discoverMediation(agentContext, mediatorId) {
        ...
        if (defaultMediator) {
            if (defaultMediator.state !== models_2.MediationState.Granted) {
                throw new core_2.CredoError(`Mediation State for ${defaultMediator.id} is not granted, but is set as default mediator!`);
            }
