...
let ProofsApi = class ProofsApi {
    ...
    getProtocol(protocolVersion) {
        ...
        if (!proofProtocol) {
            throw new core_1.CredoError(`No proof protocol registered for protocol version ${protocolVersion}`);
        }
    ...
    async acceptProposal(options) {
        ...
        if (!proofRecord.connectionId) {
            throw new core_1.CredoError(`No connectionId found for proof record '${proofRecord.id}'. Connection-less verification does not support presentation proposal or negotiation.`);
        }
    ...
    async acceptPresentation(options) {
        ...
        if (!presentationMessage) {
            throw new core_1.CredoError(`No presentation message found for proof record with id '${proofRecord.id}'`);
        }
        // Use connection if present
        const connectionRecord = proofRecord.connectionId
            ? await this.connectionService.getById(this.agentContext, proofRecord.connectionId)
