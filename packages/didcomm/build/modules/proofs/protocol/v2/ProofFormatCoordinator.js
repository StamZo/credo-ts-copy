...
class ProofFormatCoordinator {
    ...
    getAttachmentForService(credentialFormatService, formats, attachments) {
        ...
        if (!attachment) {
            throw new core_1.CredoError(`Attachment with id ${attachmentId} not found in attachments.`);
        }
        return attachment;
    }
    getAttachmentIdForService(credentialFormatService, formats) {
        ...
        if (!format)
            throw new core_1.CredoError(`No attachment found for service ${credentialFormatService.formatKey}`);
