...
class V2ProofProtocol extends BaseProofProtocol_1.BaseProofProtocol {
    ...
    async createProposal(agentContext, { connectionRecord, proofFormats, comment, autoAcceptProof, goalCode, goal, parentThreadId, }) {
        const proofRepository = agentContext.dependencyManager.resolve(repository_2.ProofRepository);
        ...
        const proofRecord = new repository_2.ProofExchangeRecord({
    ...
    async processProposal(messageContext) {
        ...
        if (proofRecord) {
            ...
            const lastSentMessage = await didCommMessageRepository.findAgentMessage(messageContext.agentContext, {
                associatedRecordId: proofRecord.id,
                messageClass: messages_2.V2RequestPresentationMessage,
        ...
        proofRecord = new repository_2.ProofExchangeRecord({
            connectionId: connection?.id,
            threadId: proposalMessage.threadId,
    ...
    async getCredentialsForRequest(agentContext, { proofRecord, proofFormats }) {
        ...
        // of the formats
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to get credentials for request. No supported formats provided as input or in request message');
        }
    ...
    async acceptPresentation(agentContext, { proofRecord }) {
        ...
        if (!presentation.lastPresentation) {
            throw new core_1.CredoError(`Trying to send an ack message while presentation with id ${presentation.id} indicates this is not the last presentation (presentation.last_presentation is set to false)`);
        }
        const message = new messages_2.V2PresentationAckMessage({
