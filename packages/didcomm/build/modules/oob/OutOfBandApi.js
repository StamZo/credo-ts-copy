...
const parseInvitation_1 = require("../../util/parseInvitation");
const connections_1 = require("../connections");
const ConnectionsApi_1 = require("../connections/ConnectionsApi");
...
let OutOfBandApi = class OutOfBandApi {
    ...
    async createInvitation(config = {}) {
        ...
        const didcommConfig = this.agentContext.dependencyManager.resolve(DidCommModuleConfig_1.DidCommModuleConfig);
        ...
        if (!handshake && !messages) {
            throw new core_1.CredoError('One or both of handshake_protocols and requests~attach MUST be included in the message.');
        }
        if (!handshake && customHandshakeProtocols) {
            throw new core_1.CredoError(`Attribute 'handshake' can not be 'false' when 'handshakeProtocols' is defined.`);
        }
        ...
        if (messages && multiUseInvitation) {
            throw new core_1.CredoError("Attribute 'multiUseInvitation' can not be 'true' when 'messages' is defined.");
        }
        ...
        if (config.routing && config.invitationDid) {
            throw new core_1.CredoError("Both 'routing' and 'invitationDid' cannot be provided at the same time.");
        }
        ...
        if (config.invitationDid) {
            ...
            services = routing.endpoints.map((endpoint, index) => {
                ...
                return new OutOfBandDidCommService_1.OutOfBandDidCommService({
                    ...
                    recipientKeys: [routing.recipientKey].map((key) => new core_1.DidKey(key).did),
                    routingKeys: routing.routingKeys.map((key) => new core_1.DidKey(key).did),
                });
            });
        }
    ...
    async createLegacyInvitation(config = {}) {
        ...
        return { outOfBandRecord, invitation: (0, converters_1.convertToOldInvitation)(outOfBandRecord.outOfBandInvitation) };
    }
    async createLegacyConnectionlessInvitation(config) {
        ...
        return {
            ...
            invitationUrl: `${config.domain}?d_m=${core_1.JsonEncoder.toBase64URL(core_1.JsonTransformer.toJSON(config.message))}`,
            outOfBandRecord,
        };
    }
    /**
    ...
    async _receiveInvitation(invitation, config = {}) {
        ...
        const outOfBandInvitation = invitation instanceof messages_1.OutOfBandInvitation ? invitation : (0, converters_1.convertToNewInvitation)(invitation);
        ...
        if (!config.isImplicit) {
            ...
            if (existingOobRecordsFromThisId.length > 0) {
                throw new core_1.CredoError(`An out of band record with invitation ${outOfBandInvitation.id} has already been received. Invitations should have a unique id.`);
            }
        }
        const recipientKeyFingerprints = await this.resolveInvitationRecipientKeyFingerprints(outOfBandInvitation);
        const outOfBandRecord = new OutOfBandRecord_1.OutOfBandRecord({
    ...
    async acceptInvitation(outOfBandId, config) {
        ...
        if (!routing && recipientRouting) {
            const recipientPublicJwk = core_1.Kms.PublicJwk.fromFingerprint(recipientRouting.recipientKeyFingerprint);
            recipientPublicJwk.keyId = recipientRouting.recipientKeyId ?? recipientPublicJwk.legacyKeyId;
            routing = {
                recipientKey: recipientPublicJwk,
                routingKeys: recipientRouting.routingKeyFingerprints.map((fingerprint) => core_1.Kms.PublicJwk.fromFingerprint(fingerprint)),
        ...
        if (handshakeProtocols && handshakeProtocols.length > 0) {
            ...
            let connectionRecord = undefined;
    ...
    async deleteById(outOfBandId) {
        ...
        if (outOfBandRecord.mediatorId &&
            ...
            const recipientKeys = outOfBandRecord
                .getTags()
                .recipientKeyFingerprints.map((item) => core_1.Kms.PublicJwk.fromFingerprint(item));
    ...
    assertHandshakeProtocolsSupported(handshakeProtocols) {
        if (!this.areHandshakeProtocolsSupported(handshakeProtocols)) {
            ...
            throw new core_1.CredoError(`Handshake protocols [${handshakeProtocols}] are not supported. Supported protocols are [${supportedProtocols}]`);
        }
    }
    ...
    getSupportedHandshakeProtocols(limitToHandshakeProtocols) {
        ...
        if (supportedHandshakeProtocols.length === 0) {
            throw new core_1.CredoError('There is no handshake protocol supported. Agent can not create a connection.');
        }
    ...
    getFirstSupportedProtocol(protocolUris) {
        ...
        if (!firstSupportedProtocol) {
            throw new core_1.CredoError(`Handshake protocols [${protocolUris}] are not supported. Supported protocols are [${supportedProtocols.map((p) => p.handshakeProtocol)}]`);
        }
    ...
    async findExistingConnection(outOfBandInvitation) {
        ...
        const invitationDids = [
            ...
            ...outOfBandInvitation.getInlineServices().map(helpers_1.outOfBandServiceToInlineKeysNumAlgo2Did),
        ];
    ...
    async emitWithConnection(outOfBandRecord, connectionRecord, messages) {
        ...
        if (!plaintextMessage) {
            throw new core_1.CredoError('There is no message in requests~attach supported by agent.');
        }
    ...
    async emitWithServices(outOfBandRecord, services, messages) {
        if (!services || services.length === 0) {
            throw new core_1.CredoError('There are no services. We can not emit messages');
        }
        const supportedMessageTypes = this.messageHandlerRegistry.supportedMessageTypes;
        const plaintextMessage = messages.find((message) => {
            const parsedMessageType = (0, messageType_1.parseMessageType)(message['@type']);
    ...
    ensureParentThreadId(outOfBandRecord, plaintextMessage) {
        ...
        if (plaintextMessage['~thread']?.pthid &&
            ...
            throw new core_1.CredoError(`Out of band invitation requests~attach message contains parent thread id ${plaintextMessage['~thread'].pthid} that does not match the invitation id ${outOfBandRecord.outOfBandInvitation.id}`);
        }
    ...
    async handleHandshakeReuse(outOfBandRecord, connectionRecord) {
        ...
        const reuseAcceptedEventPromise = (0, rxjs_1.firstValueFrom)(this.eventEmitter.observable(OutOfBandEvents_1.OutOfBandEventTypes.HandshakeReused).pipe((0, core_1.filterContextCorrelationId)(this.agentContext.contextCorrelationId), 
    ...
    async resolveInvitationRecipientKeyFingerprints(outOfBandInvitation) {
        ...
        for (const service of outOfBandInvitation.getServices()) {
            ...
            if (typeof service === 'string') {
                ...
                recipientKeyFingerprints.push(...service.recipientKeys.map((didKey) => core_1.DidKey.fromDid(didKey).publicJwk.fingerprint));
            }
        }
