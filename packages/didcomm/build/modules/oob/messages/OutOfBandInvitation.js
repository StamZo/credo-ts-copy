...
class OutOfBandInvitation extends AgentMessage_1.AgentMessage {
    ...
    addRequest(message) {
        ...
        const requestAttachment = new Attachment_1.Attachment({
            ...
            data: new Attachment_1.AttachmentData({
                base64: core_1.JsonEncoder.toBase64(message.toJSON()),
            }),
        });
    ...
    toUrl({ domain }) {
        ...
        const encodedInvitation = core_1.JsonEncoder.toBase64URL(invitationJson);
    ...
    static fromUrl(invitationUrl) {
        ...
        throw new core_1.CredoError('InvitationUrl is invalid. It needs to contain one, and only one, of the following parameters; `oob`');
    }
    static fromJson(json) {
        return core_1.JsonTransformer.fromJSON(json, OutOfBandInvitation);
    }
    get invitationDids() {
        const dids = this.getServices().map((didOrService) => {
            ...
            return (0, helpers_1.outOfBandServiceToNumAlgo2Did)(didOrService);
        });
        return dids;
    }
...
__decorate([
    ...
    (0, core_1.IsStringOrInstance)(OutOfBandDidCommService_1.OutOfBandDidCommService, { each: true }),
    __metadata("design:type", Array)
    ...
    return (0, class_transformer_1.Transform)(({ value, type }) => {
        if (type === class_transformer_1.TransformationType.PLAIN_TO_CLASS) {
            return value.map((service) => {
                ...
                return core_1.JsonTransformer.fromJSON(service, OutOfBandDidCommService_1.OutOfBandDidCommService);
            });
        }
        if (type === class_transformer_1.TransformationType.CLASS_TO_PLAIN) {
            return value.map((service) => typeof service === 'string' || service instanceof String ? service.toString() : core_1.JsonTransformer.toJSON(service));
        }
        // PLAIN_TO_PLAIN
        return value;
    });
