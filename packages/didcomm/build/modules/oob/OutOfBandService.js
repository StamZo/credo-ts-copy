...
const helpers_1 = require("../connections/services/helpers");
...
let OutOfBandService = class OutOfBandService {
    ...
    async createFromImplicitInvitation(agentContext, config) {
        ...
        const publicDid = (0, core_1.parseDid)(did);
        ...
        if (!createdDid) {
            throw new core_1.CredoError(`Referenced public did ${did} not found.`);
        }
    ...
    async processHandshakeReuse(messageContext) {
        ...
        if (!parentThreadId) {
            throw new core_1.CredoError('handshake-reuse message must have a parent thread id');
        }
        const outOfBandRecord = await this.findByCreatedInvitationId(messageContext.agentContext, parentThreadId);
        if (!outOfBandRecord) {
            throw new core_1.CredoError('No out of band record found for handshake-reuse message');
        }
        // Assert
        ...
        if (requestLength > 0) {
            throw new core_1.CredoError('Handshake reuse should only be used when no requests are present');
        }
    ...
    async processHandshakeReuseAccepted(messageContext) {
        ...
        if (!parentThreadId) {
            throw new core_1.CredoError('handshake-reuse-accepted message must have a parent thread id');
        }
        const outOfBandRecord = await this.findByReceivedInvitationId(messageContext.agentContext, parentThreadId);
        if (!outOfBandRecord) {
            throw new core_1.CredoError('No out of band record found for handshake-reuse-accepted message');
        }
        // Assert
        ...
        if (outOfBandRecord.reuseConnectionId !== reusedConnection.id) {
            throw new core_1.CredoError('handshake-reuse-accepted is not in response to a handshake-reuse message.');
        }
        this.eventEmitter.emit(messageContext.agentContext, {
            type: OutOfBandEvents_1.OutOfBandEventTypes.HandshakeReused,
            payload: {
    ...
    inlineServiceKeys) {
        for (const service of services) {
            if (typeof service === 'string') {
                ...
                return (0, helpers_1.getResolvedDidcommServiceWithSigningKeyId)(service, inlineServiceKeys);
            }
        }
        throw new core_1.CredoError('Could not extract a service from the out of band invitation.');
    }
};
