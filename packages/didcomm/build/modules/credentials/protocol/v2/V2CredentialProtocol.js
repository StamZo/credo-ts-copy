...
class V2CredentialProtocol extends BaseCredentialProtocol_1.BaseCredentialProtocol {
    ...
    async createProposal(agentContext, { connectionRecord, credentialFormats, comment, goal, goalCode, autoAcceptCredential, }) {
        ...
        const credentialRepository = agentContext.dependencyManager.resolve(repository_2.CredentialRepository);
        const formatServices = this.getFormatServices(credentialFormats);
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to create proposal. No supported formats');
        }
        const credentialRecord = new repository_2.CredentialExchangeRecord({
            connectionId: connectionRecord.id,
            threadId: core_1.utils.uuid(),
            state: models_2.CredentialState.ProposalSent,
            role: models_2.CredentialRole.Holder,
    ...
    async processProposal(messageContext) {
        ...
        const credentialRepository = agentContext.dependencyManager.resolve(repository_2.CredentialRepository);
        const didCommMessageRepository = agentContext.dependencyManager.resolve(repository_1.DidCommMessageRepository);
        ...
        let credentialRecord = await this.findByProperties(messageContext.agentContext, {
            ...
            role: models_2.CredentialRole.Issuer,
        });
        const formatServices = this.getFormatServicesFromMessage(proposalMessage.formats);
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to process proposal. No supported formats');
        }
        // credential record already exists
        if (credentialRecord) {
            const proposalCredentialMessage = await didCommMessageRepository.findAgentMessage(messageContext.agentContext, {
                associatedRecordId: credentialRecord.id,
                messageClass: messages_2.V2ProposeCredentialMessage,
                role: repository_1.DidCommMessageRole.Receiver,
            });
            const offerCredentialMessage = await didCommMessageRepository.findAgentMessage(messageContext.agentContext, {
                associatedRecordId: credentialRecord.id,
                messageClass: messages_2.V2OfferCredentialMessage,
                role: repository_1.DidCommMessageRole.Sender,
            });
            // Assert
            credentialRecord.assertProtocolVersion('v2');
            credentialRecord.assertState(models_2.CredentialState.OfferSent);
            ...
            await this.updateState(messageContext.agentContext, credentialRecord, models_2.CredentialState.ProposalReceived);
            return credentialRecord;
        }
        // Assert
        await connectionService.assertConnectionOrOutOfBandExchange(messageContext);
        // No credential record exists with thread id
        credentialRecord = new repository_2.CredentialExchangeRecord({
            connectionId: connection?.id,
            threadId: proposalMessage.threadId,
            parentThreadId: proposalMessage.thread?.parentThreadId,
            state: models_2.CredentialState.ProposalReceived,
            role: models_2.CredentialRole.Issuer,
            protocolVersion: 'v2',
        });
        await this.credentialFormatCoordinator.processProposal(messageContext.agentContext, {
    ...
    async acceptProposal(agentContext, { credentialRecord, credentialFormats, autoAcceptCredential, comment, goal, goalCode, }) {
        // Assert
        credentialRecord.assertProtocolVersion('v2');
        credentialRecord.assertState(models_2.CredentialState.ProposalReceived);
        const didCommMessageRepository = agentContext.dependencyManager.resolve(repository_1.DidCommMessageRepository);
        ...
        // of the formats
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to accept proposal. No supported formats provided as input or in proposal message');
        }
        ...
        await this.updateState(agentContext, credentialRecord, models_2.CredentialState.OfferSent);
    ...
    async negotiateProposal(agentContext, { credentialRecord, credentialFormats, autoAcceptCredential, comment, goal, goalCode, }) {
        ...
        const formatServices = this.getFormatServices(credentialFormats);
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to create offer. No supported formats');
        }
    ...
    async createOffer(agentContext, { credentialFormats, autoAcceptCredential, comment, goal, goalCode, connectionRecord, }) {
        ...
        const credentialRecord = new repository_2.CredentialExchangeRecord({
            connectionId: connectionRecord?.id,
            threadId: core_1.utils.uuid(),
            state: models_2.CredentialState.OfferSent,
            role: models_2.CredentialRole.Issuer,
            autoAcceptCredential,
            protocolVersion: 'v2',
        });
    ...
    async processOffer(messageContext) {
        ...
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to process offer. No supported formats');
        }
        // credential record already exists
        if (credentialRecord) {
            ...
            credentialRecord.assertProtocolVersion('v2');
            credentialRecord.assertState(models_2.CredentialState.ProposalSent);
            await connectionService.assertConnectionOrOutOfBandExchange(messageContext, {
            ...
            await this.updateState(messageContext.agentContext, credentialRecord, models_2.CredentialState.OfferReceived);
            return credentialRecord;
        }
        // Assert
        await connectionService.assertConnectionOrOutOfBandExchange(messageContext);
        ...
        credentialRecord = new repository_2.CredentialExchangeRecord({
            ...
            state: models_2.CredentialState.OfferReceived,
            role: models_2.CredentialRole.Holder,
            protocolVersion: 'v2',
        });
        await this.credentialFormatCoordinator.processOffer(messageContext.agentContext, {
    ...
    async acceptOffer(agentContext, { credentialRecord, autoAcceptCredential, comment, goal, goalCode, credentialFormats, }) {
        ...
        // Assert
        credentialRecord.assertProtocolVersion('v2');
        credentialRecord.assertState(models_2.CredentialState.OfferReceived);
        // Use empty credentialFormats if not provided to denote all formats should be accepted
        let formatServices = this.getFormatServices(credentialFormats ?? {});
        // if no format services could be extracted from the credentialFormats
        ...
        // of the formats
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to accept offer. No supported formats provided as input or in offer message');
        }
        ...
        credentialRecord.autoAcceptCredential = autoAcceptCredential ?? credentialRecord.autoAcceptCredential;
        await this.updateState(agentContext, credentialRecord, models_2.CredentialState.RequestSent);
    ...
    async negotiateOffer(agentContext, { credentialRecord, credentialFormats, autoAcceptCredential, comment, goal, goalCode, }) {
        ...
        credentialRecord.autoAcceptCredential = autoAcceptCredential ?? credentialRecord.autoAcceptCredential;
        await this.updateState(agentContext, credentialRecord, models_2.CredentialState.ProposalSent);
        return { credentialRecord, message: proposalMessage };
    }
    /**
    ...
    async createRequest(agentContext, { credentialFormats, autoAcceptCredential, comment, goal, goalCode, connectionRecord, }) {
        ...
        const formatServices = this.getFormatServices(credentialFormats);
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to create request. No supported formats');
        }
        const credentialRecord = new repository_2.CredentialExchangeRecord({
            connectionId: connectionRecord.id,
            threadId: core_1.utils.uuid(),
            state: models_2.CredentialState.RequestSent,
            role: models_2.CredentialRole.Holder,
            autoAcceptCredential,
            protocolVersion: 'v2',
        });
    ...
    async processRequest(messageContext) {
        ...
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to process request. No supported formats');
        }
        // credential record already exists
        if (credentialRecord) {
            ...
            await this.updateState(messageContext.agentContext, credentialRecord, models_2.CredentialState.RequestReceived);
            return credentialRecord;
        }
        // Assert
        await connectionService.assertConnectionOrOutOfBandExchange(messageContext);
        ...
        credentialRecord = new repository_2.CredentialExchangeRecord({
            ...
            state: models_2.CredentialState.RequestReceived,
            role: models_2.CredentialRole.Issuer,
            protocolVersion: 'v2',
        });
        await this.credentialFormatCoordinator.processRequest(messageContext.agentContext, {
    ...
    async acceptRequest(agentContext, { credentialRecord, autoAcceptCredential, comment, goal, goalCode, credentialFormats, }) {
        ...
        // Assert
        credentialRecord.assertProtocolVersion('v2');
        credentialRecord.assertState(models_2.CredentialState.RequestReceived);
        // Use empty credentialFormats if not provided to denote all formats should be accepted
        let formatServices = this.getFormatServices(credentialFormats ?? {});
        // if no format services could be extracted from the credentialFormats
        ...
        if (formatServices.length === 0) {
            const requestMessage = await didCommMessageRepository.getAgentMessage(agentContext, {
                associatedRecordId: credentialRecord.id,
                messageClass: messages_2.V2RequestCredentialMessage,
                role: repository_1.DidCommMessageRole.Receiver,
            });
        ...
        // of the formats
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to accept request. No supported formats provided as input or in request message');
        }
        ...
        credentialRecord.autoAcceptCredential = autoAcceptCredential ?? credentialRecord.autoAcceptCredential;
        await this.updateState(agentContext, credentialRecord, models_2.CredentialState.CredentialIssued);
        return { credentialRecord, message };
    }
    /**
    ...
    async processCredential(messageContext) {
        ...
        // Assert
        credentialRecord.assertProtocolVersion('v2');
        credentialRecord.assertState(models_2.CredentialState.RequestSent);
        await connectionService.assertConnectionOrOutOfBandExchange(messageContext, {
        ...
        if (formatServices.length === 0) {
            throw new core_1.CredoError('Unable to process credential. No supported formats');
        }
        ...
        await this.updateState(messageContext.agentContext, credentialRecord, models_2.CredentialState.CredentialReceived);
        return credentialRecord;
    }
    /**
    ...
    async acceptCredential(agentContext, { credentialRecord }) {
        credentialRecord.assertProtocolVersion('v2');
        credentialRecord.assertState(models_2.CredentialState.CredentialReceived);
        // Create message
        const ackMessage = new messages_2.V2CredentialAckMessage({
            status: messages_1.AckStatus.OK,
            threadId: credentialRecord.threadId,
        });
        ackMessage.setThread({ threadId: credentialRecord.threadId, parentThreadId: credentialRecord.parentThreadId });
        await this.updateState(agentContext, credentialRecord, models_2.CredentialState.Done);
    ...
    async processAck(messageContext) {
        ...
        const credentialMessage = await didCommMessageRepository.getAgentMessage(messageContext.agentContext, {
            associatedRecordId: credentialRecord.id,
            messageClass: messages_2.V2IssueCredentialMessage,
            role: repository_1.DidCommMessageRole.Sender,
        });
        // Assert
        credentialRecord.assertProtocolVersion('v2');
        credentialRecord.assertState(models_2.CredentialState.CredentialIssued);
        await connectionService.assertConnectionOrOutOfBandExchange(messageContext, {
        ...
        // Update record
        await this.updateState(messageContext.agentContext, credentialRecord, models_2.CredentialState.Done);
        return credentialRecord;
    }
    /**
    ...
    async createProblemReport(_agentContext, { credentialRecord, description }) {
        const message = new messages_2.V2CredentialProblemReportMessage({
            description: {
                en: description,
                code: models_2.CredentialProblemReportReason.IssuanceAbandoned,
            },
        });
    ...
    async shouldAutoRespondToProposal(agentContext, options) {
        ...
        if (autoAccept === models_2.AutoAcceptCredential.Always)
            return true;
        if (autoAccept === models_2.AutoAcceptCredential.Never)
            return false;
    ...
    getFormatServiceForRecordType(credentialRecordType) {
        ...
        if (!formatService) {
            throw new core_1.CredoError(`No format service found for credential record type ${credentialRecordType} in v2 credential protocol`);
        }
