...
class JsonLdCredentialFormatService {
    constructor() {
        ...
        this.areCredentialsEqual = (message1, message2) => {
            ...
            return core_1.utils.areObjectsEqual(obj1, obj2);
        };
    }
    /**
    ...
    async createProposal(_agentContext, { credentialFormats }) {
        ...
        if (!jsonLdFormat) {
            throw new core_1.CredoError('Missing jsonld payload in createProposal');
        }
        // this does the validation
        core_1.JsonTransformer.fromJSON(jsonLdFormat.credential, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
    ...
    async processProposal(_agentContext, { attachment }) {
        ...
        if (!credProposalJson) {
            throw new core_1.CredoError('Missing jsonld credential proposal data payload');
        }
        // validation is done in here
        core_1.JsonTransformer.fromJSON(credProposalJson, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
    }
    async acceptProposal(_agentContext, { attachmentId, proposalAttachment }) {
        ...
        core_1.JsonTransformer.fromJSON(credentialProposal, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
    ...
    async createOffer(_agentContext, { credentialFormats, attachmentId }) {
        ...
        const jsonLdFormat = credentialFormats?.jsonld;
        if (!jsonLdFormat) {
            throw new core_1.CredoError('Missing jsonld payload in createOffer');
        }
        // validate
        core_1.JsonTransformer.fromJSON(jsonLdFormat.credential, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
        const attachment = this.getFormatData(jsonLdFormat, format.attachmentId);
    ...
    async processOffer(_agentContext, { attachment }) {
        ...
        if (!credentialOfferJson) {
            throw new core_1.CredoError('Missing jsonld credential offer data payload');
        }
        core_1.JsonTransformer.fromJSON(credentialOfferJson, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
    }
    async acceptOffer(_agentContext, { attachmentId, offerAttachment }) {
        const credentialOffer = offerAttachment.getDataAsJson();
        // validate
        core_1.JsonTransformer.fromJSON(credentialOffer, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
        const format = new CredentialFormatSpec_1.CredentialFormatSpec({
            attachmentId,
            format: JSONLD_VC_DETAIL,
        });
    ...
    async createRequest(_agentContext, { credentialFormats }) {
        ...
        if (!jsonLdFormat) {
            throw new core_1.CredoError('Missing jsonld payload in createRequest');
        }
        // this does the validation
        core_1.JsonTransformer.fromJSON(jsonLdFormat.credential, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
        const attachment = this.getFormatData(jsonLdFormat, format.attachmentId);
    ...
    async processRequest(_agentContext, { attachment }) {
        ...
        if (!requestJson) {
            throw new core_1.CredoError('Missing jsonld credential request data payload');
        }
        // validate
        core_1.JsonTransformer.fromJSON(requestJson, JsonLdCredentialDetail_1.JsonLdCredentialDetail);
    }
    async acceptRequest(agentContext, { credentialFormats, attachmentId, requestAttachment }) {
        const w3cJsonLdCredentialService = agentContext.dependencyManager.resolve(core_1.W3cJsonLdCredentialService);
        ...
        if (!verificationMethod) {
            throw new core_1.CredoError('Missing verification method in credential data');
        }
        const format = new CredentialFormatSpec_1.CredentialFormatSpec({
            attachmentId,
            format: JSONLD_VC,
        ...
        if (foundFields.length > 0) {
            throw new core_1.CredoError(`Some fields are not currently supported in credential options: ${foundFields.join(', ')}`);
        }
        const credential = core_1.JsonTransformer.fromJSON(credentialRequest.credential, core_1.W3cCredential);
        ...
        const attachment = this.getFormatData(core_1.JsonTransformer.toJSON(verifiableCredential), format.attachmentId);
        return { format, attachment };
    }
    /**
    ...
    async deriveVerificationMethod(agentContext, credentialAsJson, credentialRequest) {
        ...
        const w3cJsonLdCredentialService = agentContext.dependencyManager.resolve(core_1.W3cJsonLdCredentialService);
        const credential = core_1.JsonTransformer.fromJSON(credentialAsJson, core_1.W3cCredential);
        ...
        if (!keyType || keyType.length === 0) {
            throw new core_1.CredoError(`No Key Type found for proofType ${proofType}`);
        }
        const verificationMethod = await (0, core_1.findVerificationMethodByKeyType)(keyType[0], issuerDidDocument);
        if (!verificationMethod) {
            throw new core_1.CredoError(`Missing verification method for key type ${keyType}`);
        }
    ...
    async processCredential(agentContext, { credentialRecord, attachment, requestAttachment }) {
        ...
        const credential = core_1.JsonTransformer.fromJSON(credentialAsJson, core_1.W3cJsonLdVerifiableCredential);
        ...
        if (result && !result.isValid) {
            throw new core_1.CredoError(`Failed to validate credential, error = ${result.error}`);
        }
    ...
    verifyReceivedCredentialMatchesRequest(credential, request) {
        const jsonCredential = core_1.JsonTransformer.toJSON(credential);
        jsonCredential.proof = undefined;
        if (Array.isArray(credential.proof)) {
            throw new core_1.CredoError('Credential proof arrays are not supported');
        }
        if (request.options.created && credential.proof.created !== request.options.created) {
            throw new core_1.CredoError('Received credential proof created does not match created from credential request');
        }
        if (credential.proof.domain !== request.options.domain) {
            throw new core_1.CredoError('Received credential proof domain does not match domain from credential request');
        }
        if (credential.proof.challenge !== request.options.challenge) {
            throw new core_1.CredoError('Received credential proof challenge does not match challenge from credential request');
        }
        if (credential.proof.type !== request.options.proofType) {
            throw new core_1.CredoError('Received credential proof type does not match proof type from credential request');
        }
        if (credential.proof.proofPurpose !== request.options.proofPurpose) {
            throw new core_1.CredoError('Received credential proof purpose does not match proof purpose from credential request');
        }
        // Check whether the received credential (minus the proof) matches the credential request
        if (!core_1.utils.areObjectsEqual(jsonCredential, request.credential)) {
            throw new core_1.CredoError('Received credential does not match credential request');
        }
    ...
    async shouldAutoRespondToCredential(_agentContext, { requestAttachment, credentialAttachment }) {
        ...
        const w3cCredential = core_1.JsonTransformer.fromJSON(credentialJson, core_1.W3cJsonLdVerifiableCredential);
