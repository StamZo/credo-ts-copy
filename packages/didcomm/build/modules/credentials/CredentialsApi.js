...
const DidCommMessageRepository_1 = require("../../repository/DidCommMessageRepository");
...
let CredentialsApi = class CredentialsApi {
    ...
    getProtocol(protocolVersion) {
        ...
        if (!credentialProtocol) {
            throw new core_1.CredoError(`No credential protocol registered for protocol version ${protocolVersion}`);
        }
    ...
    async acceptProposal(options) {
        ...
        if (!credentialRecord.connectionId) {
            throw new core_1.CredoError(`No connectionId found for credential record '${credentialRecord.id}'. Connection-less issuance does not support credential proposal or negotiation.`);
        }
    ...
    async negotiateProposal(options) {
        const credentialRecord = await this.getById(options.credentialRecordId);
        if (!credentialRecord.connectionId) {
            throw new core_1.CredoError(`No connection id for credential record ${credentialRecord.id} not found. Connection-less issuance does not support negotiation`);
        }
    ...
    async acceptOffer(options) {
        ...
        if (!offerMessage) {
            throw new core_1.CredoError(`No offer message found for credential record with id '${credentialRecord.id}'`);
        }
    ...
    async acceptCredential(options) {
        ...
        if (!credentialMessage) {
            throw new core_1.CredoError(`No credential message found for credential record with id '${credentialRecord.id}'`);
        }
...
exports.CredentialsApi = CredentialsApi = __decorate([
    ...
    __metadata("design:paramtypes", [MessageSender_1.MessageSender,
        ...
        core_1.AgentContext, Object, CredentialRepository_1.CredentialRepository,
        RoutingService_1.RoutingService,
        DidCommMessageRepository_1.DidCommMessageRepository,
        services_1.RevocationNotificationService,
