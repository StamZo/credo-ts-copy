...
class CredentialExchangeRecord extends core_1.BaseRecord {
    constructor(props) {
        super();
        this.credentials = [];
    ...
    assertProtocolVersion(version) {
        if (this.protocolVersion !== version) {
            throw new core_1.CredoError(`Credential record has invalid protocol version ${this.protocolVersion}. Expected version ${version}`);
        }
    }
    assertState(expectedStates) {
        ...
        if (!expectedStates.includes(this.state)) {
            throw new core_1.CredoError(`Credential record is in invalid state ${this.state}. Valid states are: ${expectedStates.join(', ')}.`);
        }
    }
    assertConnection(currentConnectionId) {
        if (!this.connectionId) {
            throw new core_1.CredoError('Credential record is not associated with any connection. This is often the case with connection-less credential exchange');
        }
        if (this.connectionId !== currentConnectionId) {
            throw new core_1.CredoError(`Credential record is associated with connection '${this.connectionId}'. Current connection is '${currentConnectionId}'`);
        }
    }
}
