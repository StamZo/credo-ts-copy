...
let MessagePickupApi = class MessagePickupApi {
    ...
    getProtocol(protocolVersion) {
        ...
        if (!protocol) {
            throw new core_1.CredoError(`No message pickup protocol registered for protocol version ${protocolVersion}`);
        }
        return protocol;
    }
    /**
    ...
    async deliverMessages(options) {
        ...
        if (!session) {
            throw new core_1.CredoError(`No active live mode session found with id ${pickupSessionId}`);
        }
...
exports.MessagePickupApi = MessagePickupApi = __decorate([
    ...
    ,
    __param(6, (0, core_1.inject)(core_1.InjectionSymbols.Stop$)),
