...
function TransportDecorated(Base) {
    class TransportDecoratorExtension extends Base {
        ...
        hasReturnRouting(threadId) {
            ...
            if (this.transport.returnRoute === TransportDecorator_1.ReturnRouteTypes.all)
                return true;
            // transport 'thread' with matching thread id is true
            if (this.transport.returnRoute === TransportDecorator_1.ReturnRouteTypes.thread && this.transport.returnRouteThread === threadId)
                return true;
