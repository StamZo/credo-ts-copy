...
let TenantAgentContextProvider = class TenantAgentContextProvider {
    ...
    async getAgentContextForContextCorrelationId(contextCorrelationId, { provisionContext = false } = {}) {
        ...
        const shouldUpdate = !(0, core_2.isStorageUpToDate)(tenantRecord.storageVersion);
        ...
        if (shouldUpdate && !this.rootAgentContext.config.autoUpdateStorageOnStartup) {
            throw new core_2.CredoError(`Current agent storage for tenant ${tenantRecord.id} is not up to date. To prevent the tenant state from getting corrupted the tenant initialization is aborted. Make sure to update the tenant storage (currently at ${tenantRecord.storageVersion}) to the latest version (${core_2.UpdateAssistant.frameworkStorageVersion}). You can also downgrade your version of Credo.`);
        }
    ...
    async getContextForInboundMessage(inboundMessage, options) {
        ...
        let tenantId = options?.contextCorrelationId &&
            ...
            : undefined;
        let recipientKeys = [];
        if (!tenantId && (0, didcomm_1.isValidJweStructure)(inboundMessage)) {
        ...
        if (!tenantId) {
            ...
            throw new core_2.CredoError("Couldn't determine tenant id for inbound message. Unable to create context");
        }
    ...
    getRecipientKeysFromEncryptedMessage(jwe) {
        const jweProtected = core_2.JsonEncoder.fromBase64(jwe.protected);
        ...
        for (const recipient of jweProtected.recipients) {
            ...
            if ((0, core_2.isJsonObject)(recipient) && (0, core_2.isJsonObject)(recipient.header) && typeof recipient.header.kid === 'string') {
    ...
    listenForRoutingKeyCreatedEvents() {
        ...
        this.eventEmitter.on(didcomm_1.RoutingEventTypes.RoutingCreatedEvent, async (event) => {
    ...
    async _updateTenantStorage(tenantRecord, agentContext, updateOptions) {
        try {
            ...
            const updateAssistant = new core_2.UpdateAssistant(tenantAgent);
...
exports.TenantAgentContextProvider = TenantAgentContextProvider = __decorate([
    (0, core_2.injectable)(),
    __param(4, (0, core_2.inject)(core_2.InjectionSymbols.Logger)),
