...
exports.supportedEncryptionAlgsForKey = supportedEncryptionAlgsForKey;
exports.allowedEncryptionAlgsForKey = allowedEncryptionAlgsForKey;
exports.assertAllowedEncryptionAlgForKey = assertAllowedEncryptionAlgForKey;
...
function supportedEncryptionAlgsForKey(jwk) {
    ...
    if (jwk.kty === 'oct') {
        ...
        if (keyBits === 256)
            algs.push('A128CBC-HS256');
        if (keyBits === 384)
            algs.push('A192CBC-HS384');
        if (keyBits === 512)
            algs.push('A256CBC-HS512');
        // For GCM/CBC we just need the exact AES key size
        if (keyBits === 128)
            algs.push('A128GCM', 'A128CBC', 'A128KW');
        if (keyBits === 192)
            algs.push('A192GCM', 'A192KW');
        if (keyBits === 256)
            algs.push('A256GCM', 'A256CBC', 'A256KW', 'C20P', 'XC20P');
    }
    return algs;
}
/**
...
function assertAllowedEncryptionAlgForKey(jwk, algorithm) {
    ...
    if (!allowedAlgs.includes(algorithm)) {
        ...
        throw new KeyManagementError_1.KeyManagementError(`${(0, humanDescription_1.getJwkHumanDescription)(jwk)} cannot be used with algorithm '${algorithm}' for content encryption or decryption.${allowedAlgsText}`);
    }
}
