...
function didToNumAlgo2DidDocument(did) {
    ...
    for (const entry of entries) {
        ...
        // Otherwise we can be sure it is a key
            ...
            const publicJwk = kms_1.PublicJwk.fromFingerprint(entryContent);
            const verificationMethods = (0, keyDidMapping_1.getVerificationMethodsForPublicJwk)(publicJwk, did);
...
function didDocumentToNumAlgo2Did(didDocument) {
    ...
    for (const [purpose, entries] of Object.entries(purposeMapping)) {
        ...
        for (const entry of dereferenced) {
            const key = (0, keyDidMapping_1.getPublicJwkFromVerificationMethod)(entry);
