...
const keyDidMapping_1 = require("../../dids/domain/key-type/keyDidMapping");
...
let W3cJwtCredentialService = class W3cJwtCredentialService {
    ...
    async verifyCredential(agentContext, options) {
        ...
        try {
            ...
            const issuerPublicKey = (0, keyDidMapping_1.getPublicJwkFromVerificationMethod)(issuerVerificationMethod);
            ...
            const issuerIsSigner = signatureResult?.jwsSigners.some((jwsSigner) => jwsSigner.jwk.fingerprint === issuerPublicKey.fingerprint);
            if (!issuerIsSigner) {
                validationResults.validations.issuerIsSigner = {
                    isValid: false,
    ...
    async verifyPresentation(agentContext, options) {
        ...
        try {
            ...
            const proverPublicKey = (0, keyDidMapping_1.getPublicJwkFromVerificationMethod)(proverVerificationMethod);
            let signatureResult = undefined;
            try {
                // Verify the JWS signature
                signatureResult = await this.jwsService.verifyJws(agentContext, {
    ...
    async resolveVerificationMethod(agentContext, verificationMethod, allowsPurposes) {
        const dids = agentContext.resolve(dids_1.DidsApi);
        const parsedDid = (0, dids_1.parseDid)(verificationMethod);
        ...
        const publicJwk = (0, keyDidMapping_1.getPublicJwkFromVerificationMethod)(verificationMethodObject);
        publicJwk.keyId =
    ...
    async getVerificationMethodForJwtCredential(agentContext, options) {
        ...
        // this is a full did url (todo check if it contains a #)
            ...
            const jwkClass = kms_1.PublicJwk.supportedPublicJwkClassForSignatureAlgorithm(credential.jwt.header.alg);
            const supportedVerificationMethodTypes = (0, keyDidMapping_1.getSupportedVerificationMethodTypesForPublicJwk)(jwkClass);
            const didDocument = await didResolver.resolveDidDocument(agentContext, signerId);
            ...
            if (verificationMethods.length === 0) {
                throw new error_1.CredoError(`No verification methods found for signer '${signerId}' and key type '${jwkClass.name}' for alg '${credential.jwt.header.alg}'. Unable to determine which public key is associated with the credential.`);
            }
            if (verificationMethods.length > 1) {
                throw new error_1.CredoError(`Multiple verification methods found for signer '${signerId}' and key type '${jwkClass.name}' for alg '${credential.jwt.header.alg}'. Unable to determine which public key is associated with the credential.`);
            }
