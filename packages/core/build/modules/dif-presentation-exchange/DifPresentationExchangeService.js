...
const lib_1 = require("@animo-id/pex/dist/main/lib");
...
let DifPresentationExchangeService = class DifPresentationExchangeService {
    ...
    validatePresentation(presentationDefinition, presentations, presentationSubmission) {
        const result = this.pex.evaluatePresentation(presentationDefinition, Array.isArray(presentations)
            ? presentations.map(utils_2.getSphereonOriginalVerifiablePresentation)
            : (0, utils_2.getSphereonOriginalVerifiablePresentation)(presentations), {
            limitDisclosureSignatureSuites: ['DataIntegrityProof.anoncreds-2023'],
            presentationSubmission,
        });
        if (result.areRequiredCredentialsPresent === pex_1.Status.ERROR) {
    ...
    formatValidated(v) {
        ...
        return validated
            .filter((r) => r.status === pex_1.Status.ERROR)
            .map((r) => r.message)
            .filter((r) => Boolean(r));
    }
    async createPresentation(agentContext, options) {
        ...
        for (const presentationToCreate of presentationsToCreate) {
            ...
            if (presentationToCreate.claimFormat === vc_1.ClaimFormat.MsoMdoc) {
                ...
                const { deviceResponseBase64Url, presentationSubmission } = await MdocDeviceResponse_1.MdocDeviceResponse.createPresentationDefinitionDeviceResponse(agentContext, {
    ...
    getSigningAlgorithmFromVerificationMethod(verificationMethod, suitableAlgorithms) {
        const publicJwk = (0, dids_1.getPublicJwkFromVerificationMethod)(verificationMethod);
    ...
    getProofTypeForLdpVc(agentContext, presentationDefinition, verificationMethod) {
        ...
        if (supportedSignatureSuites.length === 0) {
            throw new DifPresentationExchangeError_1.DifPresentationExchangeError(`Couldn't find a supported signature suite for the given jwk ${(0, kms_1.getJwkHumanDescription)(publicJwk.toJson())}`);
        }
        if (suitableSignatureSuites) {
            ...
            if (!foundSignatureSuite) {
                throw new DifPresentationExchangeError_1.DifPresentationExchangeError([
                    ...
                    `jwk type: ${(0, kms_1.getJwkHumanDescription)(publicJwk.toJson())}`,
    ...
    getPresentationSignCallback(agentContext, presentationToCreate) {
        return async (callBackParams) => {
            ...
            if (presentationToCreate.claimFormat === vc_1.ClaimFormat.SdJwtVc) {
    ...
    async queryCredentialForPresentationDefinition(agentContext, presentationDefinition) {
        ...
        if (presentationDefinitionVersion.version === lib_1.PEVersion.v1) {
        ...
        else if (presentationDefinitionVersion.version === lib_1.PEVersion.v2) {
