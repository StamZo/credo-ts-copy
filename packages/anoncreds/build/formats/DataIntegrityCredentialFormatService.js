...
class DataIntegrityCredentialFormatService {
    ...
    async createOffer(agentContext, { credentialFormats, credentialRecord, attachmentId, }) {
        ...
        const format = new didcomm_1.CredentialFormatSpec({
    ...
    async processOffer(agentContext, { attachment, credentialRecord }) {
        ...
        const dataIntegrityCredentialOffer = core_1.JsonTransformer.fromJSON(attachment.getDataAsJson(), didcomm_1.DataIntegrityCredentialOffer);
        ...
        if (missingBindingMethod) {
            throw new didcomm_1.ProblemReportError('Invalid credential offer. Missing binding method.', {
                problemCode: didcomm_1.CredentialProblemReportReason.IssuanceAbandoned,
            });
        }
    }
    async createSignedAttachment(agentContext, data, options, issuerSupportedAlgs) {
        ...
        const parsedDid = (0, core_1.parseDid)(kid);
        ...
        const keyId = keys?.find(({ didDocumentRelativeKeyId }) => didDocumentRelativeKeyId === `#${parsedDid.fragment}`)?.kmsKeyId ??
            publicJwk.legacyKeyId;
        if (alg && !publicJwk.supportedSignatureAlgorithms.includes(alg)) {
            throw new core_1.CredoError(`jwk ${publicJwk.jwkTypehumanDescription}, does not support the JWS signature alg '${alg}'`);
        }
        const signingAlg = issuerSupportedAlgs.find((supportedAlg) => publicJwk.supportedSignatureAlgorithms.includes(supportedAlg) &&
        ...
        const signedAttach = new didcomm_1.Attachment({
            mimeType: 'application/json',
            data: new didcomm_1.AttachmentData({ base64: jws.payload }),
        });
    ...
    async acceptOffer(agentContext, { credentialRecord, attachmentId, offerAttachment, credentialFormats, }) {
        const dataIntegrityFormat = credentialFormats?.dataIntegrity;
        const credentialOffer = core_1.JsonTransformer.fromJSON(offerAttachment.getDataAsJson(), didcomm_1.DataIntegrityCredentialOffer);
    ...
    async createCredentialWithAnonCredsDataIntegrityProof(agentContext, input) {
        ...
        if (!credentialSubjectIdAttribute && credentialSubjectId) {
            credentialAttributes.push(new didcomm_1.CredentialPreviewAttribute({ name: 'id', value: credentialSubjectId }));
        }
    ...
    async shouldAutoRespondToProposal(_agentContext, // biome-ignore lint/correctness/noUnusedVariables: <explanation>
    ...
    async createDataIntegrityCredentialOffer(agentContext, credentialRecord, options) {
        ...
        if (didCommSignedAttachmentBindingMethodOptions) {
            ...
            didCommSignedAttachmentBindingMethod = {
                ...
                nonce: core_1.TypedArrayEncoder.toBase64URL(kms.randomBytes({ length: 32 })),
            };
        ...
        const dataIntegrityCredentialOffer = new didcomm_1.DataIntegrityCredentialOffer({
    ...
    getSupportedJwaSignatureAlgorithms(agentContext) {
        const kms = agentContext.dependencyManager.resolve(core_1.Kms.KeyManagementApi);
        const supportedSignatureAlgorithms = Object.values(core_1.Kms.KnownJwaSignatureAlgorithms).filter((algorithm) => kms.supportedBackendsForOperation({
            operation: 'sign',
            algorithm,
        }).length > 0);
