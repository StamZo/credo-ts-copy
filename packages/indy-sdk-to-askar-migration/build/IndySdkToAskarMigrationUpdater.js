...
const askar_shared_2 = require("@openwallet-foundation/askar-shared");
...
class IndySdkToAskarMigrationUpdater {
    constructor(agent, dbPath, defaultLinkSecretId) {
        this.dbPath = dbPath;
        this.agent = agent;
    ...
    async migrate() {
        ...
        const kdfLevel = storeConfig.keyDerivationMethod ?? askar_shared_1.KdfMethod.Argon2IMod;
        // Migration tool uses the legacy key derivation method
        const keyDerivationMethod = kdfLevel === 'kdf:argon2i:mod' ? 'ARGON2I_MOD' : kdfLevel === 'kdf:argon2i:int' ? 'ARGON2I_INT' : 'RAW';
        ...
        await askar_shared_2.Migration.migrate({ specUri, walletKey, kdfLevel: keyDerivationMethod, walletName });
    }
    /*
    ...
    async update() {
        ...
        try {
            ...
            const storeConfig = this.agent.modules.askar.config.store;
            const kdfLevel = storeConfig.keyDerivationMethod ?? askar_shared_1.KdfMethod.Argon2IMod;
            this.store = await askar_shared_2.Store.open({
                uri: `sqlite://${this.backupFile}`,
                passKey: storeConfig.key,
                keyMethod: new askar_shared_1.StoreKeyMethod(kdfLevel),
            });
    ...
    async updateKeys() {
        ...
        for (;;) {
            ...
            for (const row of keys) {
                ...
                const key = askar_shared_2.Key.fromSecretBytes({
                    algorithm: askar_shared_2.KeyAlgorithm.Ed25519,
