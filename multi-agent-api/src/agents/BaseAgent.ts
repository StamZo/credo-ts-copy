...
export class BaseAgent {
  ...
  public constructor({
    ...
    // Fixed wallet path
    const walletPath = `./wallet/credo_${name}.sqlite`;  // e.g., ./wallet/credo_faber.sqlite

    // Delete old wallet file if exists
    if (fs.existsSync(walletPath)) {
      fs.unlinkSync(walletPath);
      console.log(greenText(`Cleared wallet file: ${walletPath}`));
    }
    ...
    const config = {
      ...
      walletConfig: { 
        ...
        storage: {
          type: 'sqlite',
          path: walletPath  // Set fixed path
        }
      },
