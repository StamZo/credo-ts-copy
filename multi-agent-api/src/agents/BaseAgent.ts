...
import { IndyBesuAnonCredsRegistry } from '@credo-ts/indy-besu-vdr/anoncreds'
import { IndyBesuDidRegistrar, IndyBesuDidResolver } from '@credo-ts/indy-besu-vdr/dids'
...
type DemoAgent = Agent<any> // Temporarily use 'any' to bypass type issues
...
export class BaseAgent {
  ...
  public constructor({
    ...
    this.agent = new Agent({
      ...
      modules: getCredoModules() as any, // Cast to any to bypass type checking
    })
...
function getCredoModules() {
  ...
  // Create modules without the IndyBesu module initially to avoid type conflicts
  const baseModules = {
    ...
    anoncreds: new AnonCredsModule({
      ...
      registries: [
        ...
        // Cast IndyBesuAnonCredsRegistry to bypass type checking
        new IndyBesuAnonCredsRegistry() as any
      ],
    }),
    
    askar: new AskarModule({
      askar: askarShared.askar,
      store: {
        id: 'default',
        key: 'defaultkey',
      },
    }),
    // Use askar-shared for AskarModule if required by your environment
    // If you want to use the shared bindings, you can import and use them like this:
    // import { askarShared } from '@openwallet-foundation/askar-shared'
    // askar: new AskarModule({
    //   ariesAskarInstance: askarShared,
    // }),    
  ...
  // Add IndyBesu module separately with type casting
  const indyBesuModule = new IndyBesuModule({
    chainId: 1337,
  ...
  return {
    ...baseModules,
    indyBesu: indyBesuModule as any, // Cast to bypass module type checking
  }
}