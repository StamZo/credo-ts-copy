// createFaberAgent.ts (corrected with @ts-ignore for jsonld credentialFormats)
import {
  ProofEventTypes,
  ProofState,
...
private async printConnectionInvite() {
  ...
  public async issueAnonCredsCredential(options?: { waitForAcceptance?: boolean }) {
    ...
    console.log('Issuing credential with credDefId:', this.credentialDefinition.credentialDefinitionId)
    const record = await this.agent.credentials.offerCredential({
      connectionId: connectionRecord.id,
      protocolVersion: 'v2',
      credentialFormats: { anoncreds: credential },
    })
    console.log('Credential offer sent with ID:', record.id)
  ...
  console.log('Proof request using cred_def_id:', this.credentialDefinition.credentialDefinitionId)  // Add this
